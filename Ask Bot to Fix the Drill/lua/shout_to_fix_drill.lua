...
BotFixDrill.shout2cancel = BotFixDrill.shout2cancel or {}
...
function PlayerStandard:_get_intimidation_action(prime_target, ...)
	...
	if (voice_type == "come" or voice_type == "boost") and prime_target and prime_target.unit and prime_target.unit_type == 2 and
		...
		BotFixDrill:Check_Is_He_AI(prime_target.unit) and not BotFixDrill.shout2cancel[_key] then
		...
		if not BotFixDrill.target_drill_table[_key] then
			...
			if drill then
				...
				if drill then
					BotFixDrill:Animal_Do_Fixing(prime_target.unit, true, drill)
				end
			end
		else
			BotFixDrill:Animal_End_Fixing(prime_target.unit)
			BotFixDrill.target_drill_table[_key] = {}
			BotFixDrill.target_drill_table[_key] = nil
			BotFixDrill.shout2cancel[_key] = math.round(TimerManager:game():time()) + 3
		end
	end
